...
const ArbitrageExecution = {
  ...
  attemptedPositions: {},
  telegramBot: CONFIG.TELEGRAM.ENABLED
    ? new Telegraf(CONFIG.TELEGRAM.TELEGRAM_TOKEN)
    : null,
  ...
  executeCalculatedPosition(calculated) {
    ...
    let msg = `Attempting to execute ${calculated.id} with an age of ${Math.max(
      age.ab,
      age.bc,
      age.ca
    ...
    if (telegramBot) {
      telegramBot.telegram.sendMessage(CONFIG.TELEGRAM.TELEGRAM_CHAT_ID, msg);
    }

    return ArbitrageExecution.getExecutionStrategy()(calculated)
      .then((actual) => {
        let msg = `${
          CONFIG.EXECUTION.ENABLED ? "Executed" : "Test: Executed"
        } ${calculated.id} position in ${Util.millisecondsSince(startTime)} ms`;

        logger.execution.info(msg);
        if (telegramBot) {
          telegramBot.telegram.sendMessage(
            CONFIG.TELEGRAM.TELEGRAM_CHAT_ID,
            msg
          );
        }
        ...
        msg = `BNB fees: \t  ${(-1 * actual.fees).toFixed(8)}`;
        logger.execution.info(msg);
        logger.execution.info();
      ...
      .then(() => {
        ...
        if (
          CONFIG.EXECUTION.CAP &&
          ArbitrageExecution.inProgressIds.size === 0 &&
          ArbitrageExecution.getAttemptedPositionsCount() >=
            CONFIG.EXECUTION.CAP
        ) {
          logger.execution.info(
            `Cannot exceed user defined execution cap of ${CONFIG.EXECUTION.CAP} executions`
          );
          process.exit(0);
        }
      });
  ...
  isSafeToExecute(calculated) {
    ...
    if (
      ...
      ageInMilliseconds > CONFIG.EXECUTION.THRESHOLD.AGE
    )
      return false;
    ...
    if (ArbitrageExecution.inProgressSymbols.has(symbol.a)) {
      logger.execution.trace(
        `Blocking execution because ${symbol.a} is currently involved in an execution`
      );
      return false;
    }
    if (ArbitrageExecution.inProgressSymbols.has(symbol.b)) {
      logger.execution.trace(
        `Blocking execution because ${symbol.b} is currently involved in an execution`
      );
      return false;
    }
    if (ArbitrageExecution.inProgressSymbols.has(symbol.c)) {
      logger.execution.trace(
        `Blocking execution because ${symbol.c} is currently involved in an execution`
      );
      return false;
    }
    ...
    if (
      Object.entries(ArbitrageExecution.attemptedPositions).find(
        ([executionTime, id]) =>
          id === calculated.id &&
          executionTime > Util.millisecondsSince(CONFIG.EXECUTION.THRESHOLD.AGE)
      )
    ) {
      logger.execution.trace(
    ...
    if (
      CONFIG.EXECUTION.CAP &&
      ArbitrageExecution.getAttemptedPositionsCount() >= CONFIG.EXECUTION.CAP
    ) {
      logger.execution.trace(
        `Blocking execution because ${ArbitrageExecution.getAttemptedPositionsCount()} executions have been attempted`
      );
      return false;
    }
  ...
  getAttemptedPositionsCountInLastSecond() {
    ...
    return Object.keys(ArbitrageExecution.attemptedPositions).filter(
      (time) => time > timeFloor
    ).length;
  },
  ...
  parallelExecutionStrategy(calculated) {
    return Promise.all([
      BinanceApi.marketBuyOrSell(calculated.trade.ab.method)(
        calculated.trade.ab.ticker,
        calculated.ab.quantity
      ),
      BinanceApi.marketBuyOrSell(calculated.trade.bc.method)(
        calculated.trade.bc.ticker,
        calculated.bc.quantity
      ),
      BinanceApi.marketBuyOrSell(calculated.trade.ca.method)(
      ...
      if (resultsAB.orderId && resultsBC.orderId && resultsCA.orderId) {
        [actual.a.spent, actual.b.earned, fees] =
          ArbitrageExecution.parseActualResults(
            calculated.trade.ab.method,
            resultsAB
          );
        actual.fees += fees;

        [actual.b.spent, actual.c.earned, fees] =
          ArbitrageExecution.parseActualResults(
        ...
        [actual.c.spent, actual.a.earned, fees] =
          ArbitrageExecution.parseActualResults(
  ...
  linearExecutionStrategy(calculated) {
    ...
    return BinanceApi.marketBuyOrSell(calculated.trade.ab.method)(
      calculated.trade.ab.ticker,
      calculated.ab.quantity
    )
      .then((results) => {
        ...
        return BinanceApi.marketBuyOrSell(calculated.trade.bc.method)(
          calculated.trade.bc.ticker,
          recalculated.bc
        );
      })
      .then((results) => {
        ...
        return BinanceApi.marketBuyOrSell(calculated.trade.ca.method)(
          calculated.trade.ca.ticker,
          recalculated.ca
        );
      })
